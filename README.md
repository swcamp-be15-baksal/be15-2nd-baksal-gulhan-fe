...
| 한성경 |  Vue.js를 새롭게 학습하며 개발하며 상태 관리를 위한 Pinia, Vue Router와 같은 공식 라이브러리 등을 경험할 수 있었습니다. 백엔드 개발에서 띄운 서버에서 프론트엔드 기능을 연동하였는데, Date 형식이나 별칭 등 응답값이 잘 매칭되지 않아서 백의 DTO 구조나 XML 쿼리 형식을 상황에 맞게 조율하였습니다. 이 과정에서 통신 명세의 중요성을 체감했고, 더욱 꼼꼼한 설계가 필요하다는 점을 배웠습니다. 또, Axios Interceptor를 활용해 Access Token 만료 시 자동 재발급 로직을 구현하며 인증 처리의 흐름을 실무적으로 이해할 수 있었으며, 텍스트 에디터도 처음으로 사용해보며 사용자 중심의 기능 설계에 대해 고민하는 시간을 가졌습니다.                 
