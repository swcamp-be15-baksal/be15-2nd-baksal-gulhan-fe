...
import { deleteReview } from '@/features/mypage/api2.js';
...
function goToEdit() {
  console.log('리뷰 수정페이지로..', props.review.id);
  router.push({
    path: `/mypage/review/${props.review.id}/edit`,
    query: {
      reviewId: props.review.id,
      targetId: props.review.targetId,
      targetType: props.review.targetType,
...
async function handleDelete() {
  ...
  if (confirmed) {
    try {
      await deleteReview(props.review.id, authStore.accessToken);
